...
- **Updates** — **Check for updates** and **Update & restart**, plus a *see what's in this update* compare link. Both install shapes update in-app: a **git checkout** fast-forwards to the latest commits; a **packaged (ZIP) install** downloads the latest release, backs up the current files and swaps in the new ones — keeping `data/`, `config.json`, `.env` and your `.venv` untouched — then restarts. A mid-way failure rolls back automatically, so a broken download never leaves you with a half-updated install. (If a release ever ships without a downloadable ZIP, the button falls back to a link to the releases page.)
