...
test('Escape peels one layer, not two', () => {
  ...
  assert.match(generated, /if \(repairOpen \|\| improveOpen \|\| civitaiOpen\) return;/);
  assert.match(generated,
    /if \(repairOpen \|\| improveOpen \|\| civitaiOpen\) return;[^]{0,700}if \(cameraOpen\) \{[^]{0,300}if \(e\.key === 'ArrowLeft' && onPrev\)/);
...
test('the guards are re-read when the dialog opens, not captured stale', () => {
  ...
  assert.match(generated, /\}, \[img, onClose, repairOpen, cameraOpen, improveOpen, civitaiOpen, zoom, onPrev, onNext\]\);/);
});
