  ...
  return (
    <div className="flex items-center gap-2 flex-wrap rounded-lg border border-amber-400/40 bg-amber-400/10 px-3 py-2">
      ...
      <span className="text-content text-sm">
        {t('studio.best.saved')}: <code className="text-amber-200">{best.lora_filename.split('\\').pop()}</code>
        {' '}@ <strong>{fmt(best.strength)}</strong>
      </span>
      <button type="button"
        onClick={() => { if (window.confirm(t('studio.best.deleteConfirm'))) onClear(); }}
        title={t('studio.best.deleteTitle')}
        aria-label={t('studio.best.deleteLabel')}
