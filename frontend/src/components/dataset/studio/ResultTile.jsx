    ...
    <div className="flex flex-col gap-1 items-center">
      {cell.status === 'pending' && (
        <div className="w-20 h-28 rounded-md border border-border bg-surface flex flex-col gap-2 items-center justify-center"
          role="status" aria-label={isGenerating ? t('studio.tile.generating') : t('studio.tile.queued')}>
          ...
          <span className="text-content-muted text-[0.625rem]">
            {isGenerating ? t('studio.tile.generating') : t('studio.tile.queued')}
          </span>
        </div>
      )}
      ...
      {cell.status === 'failed' && (
        <div
          title={cell.error || t('studio.tile.failedTitle')}
          ...
          <span>{t('studio.tile.failed')}</span>
          {cell.error && <span className="text-red-300/70 leading-tight line-clamp-3">{cell.error}</span>}
        </div>
      )}
      {cell.status === 'cancelled' && (
        <div className="w-20 h-28 rounded-md border border-amber-500/40 bg-amber-500/10 flex flex-col items-center justify-center text-amber-300 text-[0.625rem] gap-0.5"><span aria-hidden>⏸</span> {t('studio.tile.stopped')}</div>
      )}
      {cell.status === 'done' && cell.filename && (
        <button type="button" onClick={() => onOpen(cell)}
          title={t('studio.tile.open', {
            name: row.label,
            strength: fmt(strength),
            aspect: variant.aspect || '—',
            seed: cell.seed,
            batch: cell.batch_lora ? ` · + ${cell.batch_lora}` : '',
          })}
          ...
          <img src={`/api/dataset/${datasetId}/img/${encodeURIComponent(cell.filename)}`}
            alt={t('studio.tile.alt', { name: row.label, strength: fmt(strength), aspect: variant.aspect || '', seed: cell.seed })} loading="lazy"
      ...
      {cell.face_score != null && (
        <span title={t('studio.tile.faceSimilarity', { score: cell.face_score.toFixed(3) })}
      ...
      {cell.batch_lora && (
        <span className="max-w-[5rem] truncate px-1 py-px rounded border border-amber-400/50 bg-amber-400/15 text-amber-300 text-[0.5625rem] font-semibold"
          title={t('studio.tile.batchAxis', { name: cell.batch_lora })}>
          + {cell.batch_lora}
        </span>
      )}
      ...
      {cell.status === 'done' && (
        <div className="flex items-center gap-1">
          <button type="button" aria-pressed={cell.rating === 1}
            aria-label={t('studio.tile.like', { name: row.label, strength: fmt(strength), aspect: variant.aspect || '—', seed: cell.seed })}
          ...
          <button type="button" aria-pressed={cell.rating === -1}
            aria-label={t('studio.tile.dislike', { name: row.label, strength: fmt(strength), aspect: variant.aspect || '—', seed: cell.seed })}
            onClick={() => onRate(cell.id, cell.rating === -1 ? 0 : -1)}
