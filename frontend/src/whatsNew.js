...
export const WHATS_NEW = [
  {
    ...
    blurb:
      "When you continue a training — from its last checkpoint or an earlier, less-cooked epoch — a lineage is born: the original run, its continuation, the re-continuation, and any branch you forked off. The Runs page now draws it, two ways. Any run that's part of a lineage shows a discreet “🌳 Lineage” button, and you can flip between a compact ☰ List and a ◉ Graph — a left-to-right family tree with flowing connectors, the path to the run you're looking at lit up, and forks branching off. Either view shows each run's family, steps, dataset version and whether its LoRA is still on disk, highlights the current run, and greys a branch you resumed from an earlier step (its later saves were set aside, never deleted). Hover a run in the graph to trace its line back to the root; click any run to jump straight to its card. Your choice of view is remembered. Runs from before this shipped simply show as roots — nothing is invented.",
    to: '/cloud',
  },
  {
